...
//        $playerId = $this->authService->getPlayer()->getId();
        ...
        if(
            $this->tableRepository->currentNumberOfPlayers($tableId) + 1
                > $table->getNumberOfPlayers()
        ) {
            return $this->output->thereIsNoEmptySpots($responseModel);
        }

        if(!$this->tableRepository->addPlayer($tableId, $playerId)) {
            return $this->output->playerNotAdded($responseModel);
        }

        return $this->output->playerAdded($responseModel);
    }
}
